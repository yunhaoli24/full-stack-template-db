...
export function useGetUsersQuery(params?: MaybeRefOrGetter<UserParams | undefined>) {
  ...
  return useQuery<BackendResponse<PageData<UserDetail>>, AxiosError>({
    ...
    queryFn: async () => {
      const response = await axiosInstance.get<BackendResponse<PageData<UserDetail>>>('/sys/users', {
...
export function useUpdateUserMutation() {
  ...
  >({
    ...
    mutationFn: async ({ id, payload }) => {
      const response = await axiosInstance.put<BackendResponse<null>>(`/sys/users/${id}`, payload)
      return ensureSuccess(response.data)
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: USERS_QUERY_KEY })
